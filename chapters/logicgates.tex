...
\caption{A practical inverter (NOT) circuit. While more complicated than a simple NOT circuit, it is more efficient, wasting much less energy. It uses one P-type and one N-type \emph{field effect transistor} (FET). The diodes prevent current from $V_{in}$ flowing through the pull-up resistor ($R_4$) and the pull-down resistor ($R_1$) to ground -- it would add a little voltage to each gate all the time, effectively making the gate voltages float.}
\label{fig:cmosnot}
\end{center}
\end{figure}
